...
int precedence(const TokenData &t)
{
    ...
    if (t.token == Token::UNARY_OPERATOR)
        return 4;
    ...
    try
    {
        ...
        while (t.token != Token::NEW_LINE && t.token != Token::SEMICOLON)
        {
            ...
            else if (t.token == Token::CLOSE_PAREN)
            {
                ...
                t.token == Token::INT_literal || t.token == Token::FLOAT_literal || t.token == Token::BOOL_literal || t.token == Token::CHAR_literal || t.token == Token::STRING_literal || t.token == Token::IDENTIFIER)
            {
            ...
            else if (t.token == Token::SINGLE_OPERATOR)
            {
                ...
                while (!operator_stack.empty() && operator_stack.top().token != Token::OPEN_PAREN && precedence(operator_stack.top()) >= precedence(t))
                {
                    operand_queue.push(operator_stack.top());
                    operator_stack.pop();
                }
            ...
            else if (t.token == Token::CALL)
            {
                ...
                {
                    if (temp.token == Token::COMMA || temp.token == Token::IDENTIFIER || temp.token == Token::INT_literal || temp.token == Token::FLOAT_literal || temp.token == Token::BOOL_literal || temp.token == Token::CHAR_literal || temp.token == Token::STRING_literal)
                    {
                        operand_queue.push(temp);
                    }
                    else
    ...
    try
    {
        ...
        while (!operand_queue.empty())
        {
                ...
                t.token == Token::DOUBLE_COMPARATOR)
            {
                ...
                ast_stack.pop();
                AST_expression *left = ast_stack.top();
                ast_stack.pop();
